...
{
    // if(matrix_stack_ptr[screen] >= MATRIX_STACK_SIZE)
    //     while(1);
    m44_Copy(&MODELVIEW[screen],
             &(matrix_stack[screen][matrix_stack_ptr[screen]]));
    matrix_stack_ptr[screen]++;
}

void S3D_ModelviewMatrixPop(int screen)
{
    // if(matrix_stack_ptr[screen] < 0)
    //     while(1);
    matrix_stack_ptr[screen]--;
    m44_Copy(&(matrix_stack[screen][matrix_stack_ptr[screen]]),
             &MODELVIEW[screen]);
    global_updated[screen] = 0;
}
...
{
    if (global_updated[screen])
        return;

    global_updated[screen] = 1;
    m44_Multiply(&PROJECTION[screen], &MODELVIEW[screen],
                 &S3D_GLOBAL_MATRIX[screen]);
}
