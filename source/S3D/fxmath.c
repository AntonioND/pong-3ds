...
{
    int l = fxsqrt(fxmul(ptr_V4(v, 0), ptr_V4(v, 0))
                   + fxmul(ptr_V4(v, 1), ptr_V4(v, 1))
                   + fxmul(ptr_V4(v, 2), ptr_V4(v, 2))
                   + fxmul(ptr_V4(v, 3), ptr_V4(v, 3)));
...
void m44_CreatePerspective(m44 * m,u32 width, u32 height, s32 znear)
{
    ...
    M44SET(perspective,
           fxdiv(fxmul(float2fx(2), znear), width), int2fx(0),
           int2fx(0), int2fx(0),

           int2fx(0), fxdiv(fxmul(float2fx(2), znear), height),
           int2fx(0), int2fx(0),
                       ...
                       int znear, int zfar)
{
    ...
    M44SET(perspective,
           -fxdiv64(2 * znear, right - left), 0,
           fxdiv64(right + left, right - left), 0,

           0, fxdiv64(2 * znear, top - bottom),
           fxdiv64(top + bottom, top - bottom), 0,

           0, 0,
           fxdiv64(zfar + znear, zfar - znear),
           -fxdiv64(2 * fxmul64(zfar, znear), zfar - znear),
