...
#ifndef S3D_FXMATH_H__
#define S3D_FXMATH_H__
...
{
    return sin_lut[((theta >> 7) + (SIN_LUT_SIZE / 4)) & (SIN_LUT_SIZE - 1)] << FIX_SHIFT_EXTRA;
}

// Don't modify this, modify FIX_SHIFT_EXTRA
#define FIX_SHIFT (12 + FIX_SHIFT_EXTRA)
#define FIX_SCALE (1 << FIX_SHIFT)
#define FIX_SCALEF ((float)FIX_SCALE)
#define FIX_MASK (FIX_SCALE - 1)

// Convert an integer to fixed-point
static inline s32 int2fx(int d)
{
    return d << FIX_SHIFT;
}

// Convert a float to fixed-point
static inline s32 float2fx(float f)
{
    return (s32)(f * FIX_SCALEF);
}

// Convert a FIXED point value to an unsigned integer (orly?).
static inline u32 fx2uint(s32 fx)
{
    return fx >> FIX_SHIFT;
}

// Get the unsigned fractional part of a fixed point value (orly?).
static inline u32 fx2ufrac(s32 fx)
{
    return fx & FIX_MASK;
}

// Convert a FIXED point value to an signed integer.
static inline int fx2int(s32 fx)
{
    return fx / FIX_SCALE;
}

// Convert a fixed point value to floating point.
static inline float fx2float(s32 fx)
{
    return fx / FIX_SCALEF;
}

// Multiply two fixed point values
static inline s32 fxmul(s32 fa, s32 fb)
{
    return (fa * fb) >> FIX_SHIFT;
}

// Divide two fixed point values.
static inline s32 fxdiv(s32 fa, s32 fb)
{
    return ((fa)*FIX_SCALE) / (fb);
}

// Multiply two fixed point values using 64bit math.
static inline s32 fxmul64(s32 fa, s32 fb)
{
    return (((s64)fa) * fb) >> FIX_SHIFT;
}

// Divide two fixed point values using 64bit math.
static inline s32 fxdiv64(s32 fa, s32 fb)
{
...
{
    return fxmul64(ptr_V4(v1, 0), ptr_V4(v2, 0))
           + fxmul64(ptr_V4(v1, 1), ptr_V4(v2, 1))
           + fxmul64(ptr_V4(v1, 2), ptr_V4(v2, 2))
           + fxmul64(ptr_V4(v1, 3), ptr_V4(v2, 3));
}
...
void m44_CreateFrustum(m44 *m, int left, int right, int bottom, int top,
                       int znear, int zfar);
