...
static inline void _pad_GetBounds(_pad_t *p, int *xmin, int *xmax,
...
static void _pad_UpdateCollisions(_pad_t *p, int pad_number, int *xmargin,
                                  int *ymargin, int *zmargin)
{
    p->collisions = 0;
    ...
    _pad_GetBounds(p, &padxmin, &padxmax, &padymin, &padymax,
                   &padzmin, &padzmax);
    ...
    // X
    {
        ...
        if (padxmin <= roomxmin)
        {
            p->collisions |= COLLISION_X_MIN;
            xm = abs(roomxmin - padxmin);
        }
        else if (padxmax >= roomxmax)
        {
            p->collisions |= COLLISION_X_MAX;
            xm = abs(padxmax - roomxmax);
        }
        ...
        if (x_ball_coliding)
        {
            ...
            {
                p->collisions |= COLLISION_X_MAX;
                temp = abs(padxmax - ballxmin);
            }
            else
            {
                p->collisions |= COLLISION_X_MIN;
                temp = abs(padxmin - ballxmax);
            }

            if (temp < xm)
                xm = temp; // Save the greatest value to separate the pad
        }
    ...
    // Y
    {
        ...
        if (padymin <= roomymin)
        {
            p->collisions |= COLLISION_Y_MIN;
            ym = abs(roomymin - padymin);
        }
        else if (padymax >= roomymax)
        {
            p->collisions |= COLLISION_Y_MAX;
            ym = abs(padymax - roomymax);
        }
        ...
        if (y_ball_coliding)
        {
            ...
            {
                p->collisions |= COLLISION_Y_MAX;
                temp = abs(padymax - ballymin);
            }
            else
            {
                p->collisions |= COLLISION_Y_MIN;
                temp = abs(padymin - ballymax);
            }

            if (temp < ym)
                ym = temp; // Save the greatest value to separate the pad
        }
    ...
    // Z
    {
        ...
        if (padzmin <= roomzmin)
        {
            p->collisions |= COLLISION_Z_MIN;
            zm = abs(roomzmin - padzmin);
        }
        else if (padzmax >= roomzmax)
        {
            p->collisions |= COLLISION_Z_MAX;
            zm = abs(padzmax - roomzmax);
        }
        ...
        if (z_ball_coliding)
        {
            ...
            {
                p->collisions |= COLLISION_Z_MAX;
                temp = abs(padzmax - ballzmin);
            }
            else
            {
                p->collisions |= COLLISION_Z_MIN;
                temp = abs(padzmin - ballzmax);
            }

            if (temp < zm)
                zm = temp; // Save the greatest value to separate the pad
        }
...
#define BOUNCE_MIN_SPEED (float2fx(0.1))
    ...
    // Player 1
    {
        ...
        if (Room_3DMode() == GAME_MODE_3D)
        {
            ...
            int v = fxsqrt(fxmul(p->vx, p->vx) + fxmul(p->vy, p->vy)
                           + fxmul(p->vz, p->vz));
        ...
        if (Room_3DMode() == GAME_MODE_2D_BOUNCE)
        {
            if (p->collisions & COLLISION_Y_MIN)
            {
                ...
                {
                    p->y += ym - p->vy;
                    p->vy = -fxmul(p->vy, BOUNCE_ENERGY_CONSERVED);
    ...
    // Player 2
    {
        ...
        if (Room_3DMode() == GAME_MODE_3D)
        {
            ...
            if (Game_StateMachinePadMovementEnabled())
            {
                if ((bvz > 0) && (bz > z_first_quarter))
                {
                    // If ball is aproaching the pad, chase ball
                    ...
                    if (_segments_overlap(ballxmin, ballxmax, padxmin, padxmax)
                        < ballxsize)
                    {
                        move_left_right = 1;
                        dx = bx - p->x;
                    ...
                    if (_segments_overlap(ballymin, ballymax, padymin, padymax)
                        < ballysize)
                    {
                        move_up_down = 1;
                        dy = by - p->y;
                ...
                {
                    // If ball is leaving, go back to the center of the screen
                    int padxmin, padxmax, padymin, padymax;
                    _pad_GetBounds(p, &padxmin, &padxmax, &padymin, &padymax,
                                   NULL, NULL);

                    if (_segments_overlap(-float2fx(0.1), float2fx(0.1),
                                          padxmin, padxmax)
                        < float2fx(0.19))
                    {
                        move_left_right = 1;
