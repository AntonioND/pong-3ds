        ...
        for (j = 0; j < 4; j++)
        {
            if (ballymin < float2fx(-0.99))
            {
                if ((fast_rand() & 31) == 0)
                {
                    ...
                    int zmin = float2fx(-1.25) + float2fx(3.5)
                               + float2fx(3.5) * i + float2fx(0.1);
                    int xmax = xmin + float2fx(3.3);
                    int zmax = zmin + float2fx(3.3);

                    if (_segments_overlap(xmin, xmax, ballxmin, ballxmax)
                        && _segments_overlap(zmin, zmax, ballzmin, ballzmax))
                    {
...
static void _room_DrawRoom2_Floor(int screen)
{
    ...
    for (j = 0; j < 4; j++)
    {
        ...
        if (j & 1)
            S3D_PolygonColor(screen, 64, 64, 128);
        else
            S3D_PolygonColor(screen, 96, 96, 192);
...
void Room_2_Handle(void)
{
    if (!Game_IsPaused())
    {
        ...
        if (Game_StateMachineGet() == GAME_NORMAL_PLAY)
            Bumper_Handle();
        else if ((Game_StateMachineGet() == GAME_INITIAL_DELAY)
                 || (Game_StateMachineGet() == GAME_STARTING))
            Bumper_Init();
    }
